...
class InvoiceRetrievalWindow(BaseWindow):
    ...
    def __init__(self):
        ...
        self.email_connector = EmailConnector(self)  
        ...
        self.add_items_to_imap_servers_combo_box()
    ...
    def on_imap_server_changed(self, index):
        selected_provider = self.cmbImapServers.itemText(index)
        self.email_connector.set_imap_server(selected_provider)
    ...
    def add_items_to_imap_servers_combo_box(self):
        file_imap_servers = os.path.join(self.root_directory, 'json/imap_servers.json')
        all_imap_servers = self.email_connector.load_imap_servers(file_imap_servers) 
        self.cmbImapServers.addItems(list(all_imap_servers.keys()))  # Agrega las opciones al comboBox
        self.cmbImapServers.setCurrentIndex(0)
    ...
    def connect_and_retrieve_attachments_triggered(self):
        ...
        provider = self.cmbImapServers.currentText()
        date_start = self.dateStart.date().toString('dd/MM/yyyy')
        date_end = self.dateEnd.date().toString('dd/MM/yyyy')

        email_connector = EmailConnector(self)
        if not email_connector.set_credentials(email, password):
            print("No se pudieron establecer las credenciales de correo electrónico")
            return
        
        email_connector.set_imap_server(provider)
        email_connector.progress_updated.connect(progress_dialog.setValue)
        email_connector.status_updated.connect(self.update_status_bar)
    ...
    def offline_email_processing(self):
        ...
        # Validar la API key de OpenAI
        api_key = params.get('api_key')  
        if not Utils.validate_openai_api_key(api_key):
            return

        # Validar las rutas
        paths_to_check = [
            self.txtPathEmailsEml.text(),
            self.txtPathEmailsMsg.text(),
            self.txtExportData.text(),
        ]
        if not Utils.validate_paths(paths_to_check):
            return

        # Validar el archivo
        tesseract_path = self.txtTesseractPath.text()
        if not Utils.validate_file(tesseract_path):
            return
    ...
    def download_attachment(self):
        ...
        if ( params['email_file_type'] == '.eml' ):
            path = params[ 'path_emails_eml' ]
        else:
            path = params[ 'path_emails_msg' ]

        fp = FileProcessor.process_download(path, params[ 'path_export_xls' ])
        
    def get_params_interfaz(self):
        
        #prompts = [prompt['PROMPT'].replace('\\n', '\n') for prompt in self.config['PROMPTS']]
        # Para los prompts opcionales
        #optional_prompts = [prompt['PROMPT'].replace('\\n', '\n') for prompt in self.config['OPTIONAL_PROMPTS']]
        prompts = [{"ID": prompt['ID'], "LABEL": prompt['LABEL'], "PROMPT": prompt['PROMPT'].replace('\\n', '\n')} for prompt in self.config['PROMPTS']]
        optional_prompts = [{"ID": prompt['ID'], "LABEL": prompt['LABEL'], "PROMPT": prompt['PROMPT'].replace('\\n', '\n')} for prompt in self.config['OPTIONAL_PROMPTS']]
